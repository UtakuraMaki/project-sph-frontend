  ...
  <div>
    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            ...
            <li class="with-x" v-if="searchData.keyword">
              {{ searchData.keyword }}<i @click="removeKeyword">×</i>
            </li>
        ...
        <!--selector-->
        <SearchSelector @getTradeMark="onGetTradeMark" @getProps="onGetProps" />

        <!--details-->
        <div class="details clearfix">
          <div class="sui-navbar">
            <div class="navbar-inner filter">
              <ul class="sui-nav" @click="switchOrder">
          ...
          <div class="goods-list">
            <ul class="yui3-g">
              <li
                class="yui3-u-1-5"
                v-for="item in $store.getters.goodsList"
                :key="item.id"
              >
                <div class="list-wrap">
                  <div class="p-img">
          ...
          <Pagination
            ...
            :total="$store.state.search.list.total"
            :continues="5"
            @switchPageNo="onSwitchPageNo"
          ></Pagination>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    dispatchList() {
      this.$store.dispatch("list", this.searchData);
    },
    removeCategoryName() {
    ...
    removeKeyword() {
      ...
      Bus.$emit("removeKeyword");
    },
    onGetTradeMark(obj) {
      this.searchData.trademark = `${obj.tmId}:${obj.tmName}`;
      this.dispatchList();
    },
    removeTradeMark() {
    ...
    onGetProps(obj) {
      if (
        this.searchData.props.indexOf(
          `${obj.attrId}:${obj.attrValue}:${obj.attrName}`
        ) === -1
      ) {
        this.searchData.props.push(
          `${obj.attrId}:${obj.attrValue}:${obj.attrName}`
        );
        this.dispatchList();
      }
    ...
    switchOrder(e) {
      if (e.target.dataset.orderprop) {
        if (this.searchData.order.indexOf(e.target.dataset.orderprop) !== -1) {
          if (this.isAsc) {
            this.searchData.order = `${e.target.dataset.orderprop}:desc`;
          } else if (this.isDesc) {
            this.searchData.order = `${e.target.dataset.orderprop}:asc`;
          }
        } else {
          this.searchData.order = `${e.target.dataset.orderprop}:desc`;
        }
  ...
  watch: {
    $route: {
      // $route是只读对象，每次切换都会生成一个新的，所以不需要深度监听
      handler() {

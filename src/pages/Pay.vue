        ...
        <div class="paymark">
          <span class="fl"
            ...
            >之内完成支付，超时订单会自动取消。订单号：<em>{{
              $store.state.pay.orderId
            }}</em></span
          >
          <span class="fr"
            ><em class="lead">应付金额：</em
            ><em class="orange money"
              >￥{{ $store.state.pay.createNative.totalFee }}</em
            ></span
          >
        </div>
      </div>
...
import { getQueryPayStatus } from "@/api/index.js";
export default {
  name: "Pay",
  data() {
    return {
  ...
  mounted() {
    this.$store.dispatch("createNative", this.$store.state.pay.orderId);
  },
  methods: {
    open() {
      QRCode.toDataURL(this.$store.state.pay.createNative.codeUrl)
        .then((url) => {
          ...
          if (!this.interval) {
            this.interval = setInterval(async () => {
              const result = await getQueryPayStatus(
